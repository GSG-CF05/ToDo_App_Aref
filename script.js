...
function loadElems(){
    list.querySelectorAll('.listDiv').forEach((elem)=>{elem.remove();})
    tasks = JSON.parse(localStorage.getItem('tasks'))
    tasks.forEach((item) => loadElem(item))
}
function loadElem(item) {
...
function editer(elem) {
    ...
    form2.querySelector('button').addEventListener('click', (e) => {
        e.preventDefault();
